...
export docs_index_start=$(awk '/var doc = `{/{print NR}' docs/docs.go)
export docs_index_finish=$(awk '/}`/{print NR}' docs/docs.go)

echo ""
echo "Delete lines between $docs_index_start and $docs_index_finish"
echo "sed -i '$docs_index_start,$docs_index_finish d' docs/docs.go"

sed -i ${docs_index_start},${docs_index_finish}d docs/docs.go

if [ ! $? -eq 0 ]; then
  echo "Failure on Delete Lines"
  exit 1
fi

sed -i '"$docs_index_start" a "$http_body"' docs/docs.go
